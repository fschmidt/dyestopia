...
function card(task: Task): string {
  ...
  const labels = task.labels.length ? ` · ${task.labels.map((l) => `_${l}_`).join(', ')}` : ''
  return `- \`${task.id}\` [${task.title}](${link})${labels}`
}

function section(heading: string, cards: Task[]): string {
  const body = cards.length ? cards.map(card).join('\n') : '_Empty._'
  return `${heading}\n\n${body}`
}

function renderBoard(tasks: Task[]): string {
  const byOrdinal = (a: Task, b: Task) => a.ordinal - b.ordinal
  const ideas = tasks.filter((task) => task.type === 'idea').sort(byOrdinal)
  const work = tasks.filter((task) => task.type === 'task')

  const byLane = new Map<Lane, Task[]>(LANES.map((lane) => [lane, []]))
  for (const task of work) byLane.get(task.status)?.push(task)
  for (const lane of LANES) byLane.get(lane)!.sort(byOrdinal)

  const queued = byLane.get('Todo')!.length
  if (queued > TODO_LIMIT) {
    fail(
      `Todo holds ${queued} tasks, over the limit of ${TODO_LIMIT}. Defer the lowest-priority ` +
        `${queued - TODO_LIMIT} (set \`status: Deferred\`) — the queue is meant to be a shortlist.`,
    )
  }

  // The pipeline is the board proper; Deferred and Ideas are holding areas.
  const height = Math.max(...PIPELINE.map((lane) => byLane.get(lane)!.length), 1)
  const grid: string[][] = []
  for (let row = 0; row < height; row++) {
    grid.push(
      PIPELINE.map((lane) => {
        const task = byLane.get(lane)![row]
        return task ? `\`${task.id}\` ${task.title}` : ' '
      }),
    )
  }

  const pipeline = PIPELINE.map((lane) =>
    section(`### ${lane} (${byLane.get(lane)!.length}${lane === 'Todo' ? `/${TODO_LIMIT}` : ''})`, byLane.get(lane)!),
  ).join('\n\n')
  ...
  return [
    ...
    '## Deferred',
    '',
    `Real work, consciously postponed. Nothing here is scheduled. Todo is capped at ${TODO_LIMIT}, so`,
    'promoting something from here usually means demoting something else.',
    '',
    section(`### Deferred (${byLane.get('Deferred')!.length})`, byLane.get('Deferred')!),
    '',
    '## Ideas',
    '',
    ...
    '',
    section(`### Ideas (${ideas.length})`, ideas),
    '',
  ].join('\n')
}
...
/**
 * The index of a record directory, grouped by status. Empty states are left out
 * — six headings over two documents reads as ceremony rather than structure.
 */
function renderRecords(
  ...
  const groups = states
    .map((state) => [state, records.filter((record) => record.status === state)] as const)
    .filter(([, group]) => group.length > 0)
    .map(([state, group]) => {
      const lines = group.map((record) => {
        const cards = record.tasks.length ? ` · ${record.tasks.map((t) => `\`${t}\``).join(', ')}` : ''
        return `- \`${record.id}\` [${record.title}](${basename(record.file)})${cards}`
      })
...
/**
 * A `C-001` or `D-001` mentioned anywhere in the docs must resolve. Ids are how
 * a card points at its thinking, and an id is exactly the kind of reference that
 * survives a rename in prose while pointing at nothing.
 */
function checkRecordRefs(records: Record_[]): void {
  const known = new Set(records.map((record) => record.id))
  for (const file of [...docFiles(), 'AGENTS.md', 'CLAUDE.md']) {
    if (!existsSync(join(ROOT, file))) continue
    // The record's own frontmatter and the generated indexes are definitions,
    // not references.
    if (file.startsWith(`${CONCEPT_DIR}/`) || file.startsWith(`${DECISION_DIR}/`)) continue
    const source = read(file).replace(/```[\s\S]*?```/g, '')
    for (const [, id] of source.matchAll(/`([CD]-\d{3})`/g)) {
      if (!known.has(id)) fail(`${file}: references \`${id}\`, which is not a record on disk`)
    }
  }
}

/** A record naming a card that does not exist is a link that quietly rotted. */
function checkRecordTasks(records: Record_[], tasks: Task[]): void {
  const known = new Set(tasks.map((task) => task.id))
  for (const record of records) {
    for (const id of record.tasks) {
      if (!known.has(id)) fail(`${record.file}: lists task ${id}, which does not exist`)
    }
  }
}
...
const concepts = loadRecords('concept', fail)
const decisions = loadRecords('decision', fail)
if (concepts.length) {
  write(
    `${CONCEPT_DIR}/index.md`,
    renderRecords(concepts, CONCEPT_STATES, 'Concepts', [
...
if (decisions.length) {
  write(
    ...
    renderRecords(decisions, DECISION_STATES, 'Decisions', [
...
checkRecordTasks([...concepts, ...decisions], tasks)

applyBlocks()
applyPins()
checkPaths()
checkRecordRefs([...concepts, ...decisions])
...
console.log(CHECK ? 'Wiki is current.' : `Wiki updated (${tasks.length} tasks, ${BLOCKS.length} blocks).`)
